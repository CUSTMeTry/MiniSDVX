...
static int8_t CDC_Receive(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len) {
    ...
    if (Buf[0] == 0xFE) {
        if (HAL_I2C_Mem_Read(&hi2c1, 0xA0, 0, I2C_MEMADD_SIZE_8BIT, &sendData[1], 11, 1000) == HAL_OK) {
            sendData[0] = 0xFE;
        } else {
            sendData[0] = 0xFF;
        }
        CDC_Transmit(cdc_ch, &sendData[0], 12);
    }
    ...
    if (Buf[0] == 0xFC && *Len > 9) {
        ...
        CDC_Transmit(cdc_ch, &sendData[0], 1);
    }
